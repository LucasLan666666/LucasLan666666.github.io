...
frp 工具采用 Go 语言开发。一方面 Go 语言语法简单，易于学习；另一方面，得益于`goroutine`机制，
Go 程序会智能地将`goroutine`中的任务合理地分配给每个 CPU，轻易实现**高效的并发编程**。对于
Java/C++，不仅需要开发者维护线程池，包装一个个的任务，还需要自己去调度线程执行任务并进行上下
文切换。Go 语言之所以被称为现代化的编程语言，就是因为它在语言层面已经内置了调度和上下文切换的
机制。
